...
export function LanguageUsed({userHandle}: {userHandle: { userHandle: string };}){
    ...
    const fetchData = async () => {
      ...
      try {
        const storedData = localStorage.getItem(
          `Programing_Language-${userHandle.userHandle}`
        );
        if (storedData) {
          const localdata = JSON.parse(storedData);
          setUserData(localdata.data);
          setGuidance(localdata.guidance);
          setLoading(false);
          return;
        }
        try {
          const response = await axios.post(`${BACKEND_URL}/programming-lang`, {
            userhandle: userHandle.userHandle,
          });
          const responseData: Data = response.data as Data;
          ...
          setGuidance(responseData.guidance);
        } catch (error) {
          console.error("Error fetching programming language data:", error);
          setError("Failed to fetch programming language data.");
        } finally {
          setLoading(false);
        }
  ...
  return (
    <div >
      <div className="text-5xl text-center m-2 font-serif font-bold text-[#c9b57a]">Insights On Programing Language Used</div>
